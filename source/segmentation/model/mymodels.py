...
def load_checkpoint(checkpoint_path, hist_model):
    ...
    return {'model': hist_model['model'], 'optimizer': hist_model['optimizer'], 'start_epoch': start_epoch, 'train_dict': train_dict,'val_dict': val_dict}

def set_model(lr, model):
    optimizer = Adam(model.parameters(), lr)
    scheduler = lr_scheduler.CosineAnnealingLR(optimizer, T_max=10)
    loss_fn = ComboLoss()
    return {'model': model,'optimizer': optimizer, 'scheduler': scheduler, 'loss_fn': loss_fn}
