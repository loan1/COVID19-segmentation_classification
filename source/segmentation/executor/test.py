...
def test(dataloader, device, model, metric_fn):   
    with torch.no_grad():
        ...
        print ('jaccard: {:.4f} - acc: {:.4f} - val_f1: {:.4f} - recall: {:.4f} - precision: {:.4f}'.format (metrics_score['jaccard'], metrics_score['acc'], metrics_score['f1'], metrics_score['recall'], metrics_score['precision']))
